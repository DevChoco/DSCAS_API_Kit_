...
class RetrievalFilter(AbstractFilter):
    ...
    def execute(self, input, device=None, verbose=False):
        ...
        # query에 해당하는 카테고리만 허용
        allowed_category = self.query.lower().strip().replace(" ", "_")  # 예: "Upper Body" -> "upper_body"
    
        for cloth_path in cloth_paths:
            cloth_path_tokens = cloth_path.split('/')
            category = cloth_path_tokens[-3]  # 예: dresses, upper_body, lower_body
            cloth_filename = cloth_path_tokens[-1]
    
            if category != allowed_category:
                continue  # 지정된 카테고리가 아닌 경우 제외
